...
from tgbot.keyboards.client_keyboards import make_menu_callback_data
...
class InlineKeyboardPaginator:
    ...
    def __init__(self, page_count, current_page, level, category_id, saved_page=0):
        ...
        for key in keys:
            keyboard.append(
                InlineKeyboardButton(
                    ...
                    callback_data=make_menu_callback_data(level=self.level, category_id=self.category_id, page=key, saved_page=self.saved_page)
                )
            )
